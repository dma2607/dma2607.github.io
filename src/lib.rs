...
fn mid_point(point1: (f64, f64), point2: (f64, f64)) -> (f64, f64) {
...
fn sierpinski(context: &web_sys::CanvasRenderingContext2d, points: [(f64, f64); 3], color:(u8, u8, u8), depth: u8) {
    ...
    if depth > 0 {
        ...
        let rigth_middle = mid_point(top, right);
        ...
        // .
        sierpinski(&context, [top, left_middle, rigth_middle], next_color, depth);
        sierpinski(&context, [left_middle, left, bottom_middle], next_color, depth);
        sierpinski(&context, [rigth_middle, bottom_middle, right], next_color, depth);
        // sierpinski(&context, [(300.0, 0.0), (150.0, 300.0), (450.0, 300.0)], depth);
